...
public class PortfolioDetailsWidget extends Composite implements ClickHandler {
	...
	public void onClick(ClickEvent event) {
		if (event.getSource() == refreshButton) {
			...
			moveToNewPortfolio(portFolioId+1);
	...
	private void moveToNewPortfolio(int portfolioId) {
		History.newItem("portfolio:id="+portfolioId);
	}
	...
	public void show(int changeToPortfolio) {
		if (this.portFolioId != changeToPortfolio) {
			LOGGER.info("changing portfolio to " + portFolioId + " -> " + changeToPortfolio);
			portFolioId = changeToPortfolio;
			refresh();
		} else {
			LOGGER.info("ignoring change portfolio as it is same as current " + portFolioId + " -> " + changeToPortfolio);
		}
	}
